...
def test_parallel_constant_data_preserves_historical_interpolator_failure_boundary():
    app = _bare_app()
    frames = OrderedDict(
        (
            f"singular{i}.fcs",
    ...
    # AP06's gaussian_kde constructor succeeds for this exact constant case;
    # the historical Density path then raises from RegularGridInterpolator.
    # AP07 captures that worker exception and re-raises it only on consumption.
    assert [result.action for result in density.values()] == ["error", "error"]
    assert all(isinstance(result.error, ValueError) for result in density.values())

    # Contour's probability-independent surface still computes, and the same
    # historical interpolator error occurs later in the main-thread probability
    # classification step.
    assert [result.action for result in contour.values()] == ["payload", "payload"]
    x = frames["singular0.fcs"]["X"].to_numpy(copy=False)
    y = frames["singular0.fcs"]["Y"].to_numpy(copy=False)
    xt, yt, valid = app._transform_xy_cached("singular0.fcs", x, y)
    try:
        app._plot_contour(
            x, y, xt, yt, valid, "#123456", "c", 2, 0.5, 0.05,
            _cache_path="singular0.fcs", _precomputed=contour["singular0.fcs"],
        )
    except ValueError as exc:
        assert "strictly ascending or descending" in str(exc)
    else:
        raise AssertionError("expected the historical contour interpolation error")
    assert app._density_cache == {}
