...
# -*- coding: utf-8 -*-
"""
Flow Cytometry Visualization Tool - v4.1.8
@author: vincentpb

Changelog v4.1.7 -> v4.1.8
------------------------------
SIDEBAR RESIZING + FILE CONTEXT REVEAL

  Feat 1  The main file/control sidebar and plot area now live in a horizontal
          draggable PanedWindow. Drag the vertical divider to widen or narrow
          the sidebar; a minimum width is retained for both panes. The inner
          scrollable canvas now follows the pane width instead of remaining
          hard-coded at 320 px, so long file names actually gain usable space.

  Feat 2  Right-click (and macOS Control-click) any loaded or excluded file row
          to reveal that exact source file in Finder, Windows Explorer, or the
          Linux file manager. This reuses vFlow's existing platform-aware reveal
          implementation and never modifies, moves, renames, or deletes files.

  Safety  No data loading, gating, axis-resolution, statistics, export, or
          scientific calculation logic was changed. These are UI-only additions.

Changelog v4.1.6 -> v4.1.7
------------------------------
CHANNEL-ALIAS RESOLVER CORRECTION

  Fix 1   Alias candidates are now derived relative to the selected Main
          channel's exact measurement templates. This preserves multi-token
          channel spellings such as VGLUT1_Venus instead of collapsing them
          to the final underscore token (Venus).

  Fix 2   Real channels that coexist with the Main channel (for example D1R
          beside VGLUT1-Venus) are hidden from the normal alias-candidate list.
          They can optionally be shown for diagnostics, but are never strong
          suggestions.

  Feat 1  Separator-only channel variants such as VGLUT1-Venus,
          VGLUT1_Venus and VGLUT1 Venus are recognized as near-exact channel
          spellings while keeping the non-channel part of each column label
          byte-for-byte exact. Shorter aliases such as Venus remain available
          for explicit user confirmation when they occupy the same exact
          measurement slot in files where the Main channel is absent.

  Safety  Mapping still changes DataFrame column labels only. Numeric values,
          event order/counts, gates, masks, denominators and source files are
          not modified. Existing collision guards remain authoritative.

Changelog v4.1.5 -> v4.1.6
------------------------------
CHANNEL-AWARE NOMENCLATURE RESOLUTION

  Feat 1  The resolver now works at the CHANNEL-FAMILY level by default.
          Non-channel parts of a measurement name must match exactly. One
          confirmation such as vGAT -> VGAT can reconcile the complete exact
          structural family while avoiding fuzzy matching of unrelated fields.

  Safety  Channel suggestions require exact shared structural templates; name
          similarity is advisory only. Numeric data and source files remain
          untouched. The v4.1.5 individual-column resolver remains available
          as an Advanced fallback.

  Feat 2  Resolver now reports files whose exact schema still differs from the
          dominant resolved schema. Unresolved files can be reviewed, revealed
          in Finder / Windows Explorer / the Linux file manager, or unloaded
          from active analysis without deleting or modifying source files.

Changelog v4.1.4 -> v4.1.5
------------------------------
AXIS / CHANNEL NAME RESOLUTION

  Fix 1   Multi-file axis discovery is now diagnosable rather than appearing
          to lose channels silently.  Axis menus intentionally use the exact
          column-name intersection across all loaded files; with many files,
          one spelling variant in one file can therefore hide an otherwise
          shared axis.  A new resolver shows the union, per-name file counts,
          similarity-ranked variants, and same-file conflicts.

  Feat 1  New "Resolve Axis Names…" workflow.  The user chooses a Main Axis
          Name and one or more discovered variants to map to it.  Similar
          names are ranked and high-confidence naming variants are preselected,
          but no fuzzy match is ever applied without explicit user action.

  Safety   Resolution changes column LABELS only.  Event values, row counts,
          gate geometry, masks, denominators, and source files are untouched.
          If two selected variants coexist in the same file while the main
          name is absent, that file is left unchanged rather than creating
          duplicate/ambiguous columns.

  Fix 2   Confirmed axis aliases are now reused when Batch Stats re-reads raw
          files from disk.  Previously the existing case-only correction was
          limited to the interactive load path, so plotting could succeed but
          batch export could later report the same channel as missing.

  Fix 3   Excluding or restoring a file now recomputes the strict shared-axis
          intersection immediately.  Previously the file set changed but the
          axis menus could retain the old intersection until another load.
...
if __name__ == '__main__':
    try:
        ...
        _splash = _SplashScreen(version="4.1.7", total_steps=7)
    except Exception:
        _splash = None
...
# ─────────────────────────────────────────────────────────────────────────────
#  Axis / channel nomenclature resolution helpers
# ─────────────────────────────────────────────────────────────────────────────

def _axis_name_compact(name: str) -> str:
    """Return a conservative comparison key used only for *suggesting* aliases.

    This function never renames a channel by itself.  It intentionally removes
    common separator/punctuation differences so names such as ``CD3-A``,
    ``CD3_A`` and ``CD3 A`` rank as likely variants in the resolver UI.
    """
    text = str(name).strip().casefold()
    return ''.join(ch for ch in text if ch.isalnum())


def _axis_name_similarity(a: str, b: str) -> float:
    """Similarity score in [0, 1] for resolver ranking only.

    Exact case-insensitive matches score 1.0; separator-only variants score
    just below 1.0.  Fuzzy matching is deliberately advisory: the user must
    explicitly confirm every mapping before any DataFrame label is changed.
    """
    sa = str(a).strip().casefold()
    sb = str(b).strip().casefold()
    if sa == sb:
        return 1.0
    ca = _axis_name_compact(sa)
    cb = _axis_name_compact(sb)
    if ca and ca == cb:
        return 0.995
    # Compare both the literal normalized spelling and the compact spelling.
...
def _discover_channel_schema(columns):
    """Return {channel_name: {exact structural templates...}} for column labels.

    A template contains the literal token ``{channel}``. The detector is
    intentionally conservative about structure: it never fuzzes the
    non-channel part of a label. vSynApp-style coordinate labels are the
    strongest source of channel names (X_<channel>_microns /
    Y_<channel>_microns). Other labels are assigned to a channel only when
    they end with that exact channel token.
    """
    ...
    # Infer multi-token channel suffixes conservatively from repeated exact
    # measurement families.  If both ``Venus`` and ``VGLUT1_Venus`` occur as
    # suffix candidates in the same number of distinct labels, keep the longer
    # repeated suffix.  This prevents an underscore *inside* a channel name
    # from being mistaken for the measurement/channel delimiter.
    suffix_support = {}
    for label in labels:
    ...
    for label in labels:
        if label in coord_rows or '_' not in label or label.endswith('_'):
            continue
        # A repeated longer suffix already explains this label; do not also
        # add its last underscore token as a separate pseudo-channel.
        if any(label.endswith('_' + token) for token in repeated_pruned):
            continue
        token = label.rsplit('_', 1)[-1]
        if not token or token.casefold() in ignored:
            continue
        # If a coordinate field established a longer channel containing this
        # suffix, keep the longer exact channel name.
    ...
    for label in labels:
        if label in coord_rows:
            ...
            continue
        # Longest channel first prevents TH from stealing pTH, etc.
        for ch in candidates:
            suffix = '_' + ch
            if label.endswith(suffix) and len(label) > len(suffix):
                prefix = label[:-len(ch)]  # retains the underscore
                result.setdefault(ch, set()).add(prefix + '{channel}')
                break
    return result


def _replace_channel_in_template(template: str, channel: str) -> str:
    return str(template).replace('{channel}', str(channel))


def _extract_channel_from_template(label: str, template: str):
    """Extract the literal channel substring occupying ``{channel}``.

    The prefix and suffix around ``{channel}`` must match *exactly*.  This is
    deliberately stricter than fuzzy name comparison: only the channel slot is
    permitted to vary.  For example, template ``X_{channel}_microns`` extracts
    ``VGLUT1_Venus`` intact from ``X_VGLUT1_Venus_microns``.
    """
    marker = '{channel}'
...
def _channel_relation(canonical: str, candidate: str) -> str:
    ...
    if a.casefold() == b.casefold():
        return 'case only'
    ca = _axis_name_compact(a)
    cb = _axis_name_compact(b)
    if ca and ca == cb:
...
def _summarise_names(names, limit=4):
...
class FlowApp:
    def __init__(self, root: tk.Tk, container=None,
        ...
        # User-confirmed axis aliases: variant label -> canonical Main Axis Name.
        # Labels only; numeric data are never transformed by this mapping.
    ...
    def _build_ui(self):
        ...
        # Make the actual sidebar contents track the draggable pane width.
        # This is essential for long file names: widening the pane must widen
        # the embedded Frame too, not merely expose blank canvas space.
    ...
    def _build_controls(self):
        ...
        self._btn("Resolve Channel / Axis Names…", self.open_axis_name_resolver, 'Gray.TButton')
    ...
    def _axis_name_inventory(self) -> dict:
        """Return {exact_column_name: {file_paths...}} for currently loaded files."""
        inventory = {}
    ...
    def _channel_family_inventory(self) -> dict:
        """Return channel-level inventory using exact non-channel templates.

        Result format:
          channel -> {'files': set(paths), 'templates': set(template strings),
                      'by_file': {path: set(templates)}}
        """
        inventory = {}
        for path, df in self.loaded_files.items():
    ...
    def _channel_templates_for_name(self, channel, include_excluded=False):
        ...
        The normal schema parser remains the primary source.  A conservative
        direct pass additionally preserves full multi-token channel names when
        underscores occur *inside* the channel spelling.
        """
        ...
        for store in stores:
            for _path, df in store.items():
                ...
                for raw in df.columns:
                    label = str(raw)
                    # Exact suffix form: Measurement_<channel>
                    suffix = '_' + channel
                    if label.endswith(suffix) and len(label) > len(suffix):
                        templates.add(label[:-len(channel)] + '{channel}')
                    # Coordinate form: X_<channel>_microns / Y_<channel>_microns
                    for axis in ('X', 'Y'):
                        prefix = axis + '_'
                        tail = '_microns'
    ...
    def _channel_variants_for_canonical(self, canonical_channel):
        """Discover literal channel spellings occupying the Main channel slots.

        This is the key v4.1.7 correction.  Once a Main channel is selected,
        its exact measurement templates define where the channel substring is
        allowed to vary.  We then extract that substring intact from every
        loaded file.  Therefore ``VGLUT1_Venus`` is discovered as a whole
        spelling rather than being reduced to ``Venus`` merely because it
    ...
    def _apply_channel_mapping(self, canonical_channel, variant_channels):
        """Map confirmed literal channel aliases inside exact Main templates.

        Every character outside the ``{channel}`` slot must match exactly.
        The selected variant itself may contain underscores, hyphens or spaces;
        it is extracted and replaced as one literal channel substring.  The
        existing full-column collision guard remains authoritative.
        """
        ...
        exact_pairs = {}
        pair_conflicts = []
        # Inspect both active and excluded in-memory files so a confirmed
        # mapping remains internally consistent everywhere.
        for store in (self.loaded_files, self.excluded_files):
            for path, df in store.items():
                if df is None:
                    continue
                for template in templates:
                    target = _replace_channel_in_template(
                        template, canonical_channel)
                    for raw in df.columns:
                        source = str(raw)
        ...
        # Route older aliases that end at a newly remapped source into the new
        # exact target, preserving Batch Stats / future-load consistency.
        for alias, target in list(self.axis_aliases.items()):
            resolved = self._resolve_axis_alias_target(target)
            if target in sources:
                self.axis_aliases[alias] = exact_pairs[target]
            elif resolved in sources:
                self.axis_aliases[alias] = exact_pairs[resolved]
        for source, target in exact_pairs.items():
            self.axis_aliases[source] = target
            self.axis_aliases.pop(target, None)
        ...
        for store in (self.loaded_files, self.excluded_files):
            for path in list(store.keys()):
                ...
                if details['renamed']:
                    ...
                    changed_files += 1
                    store[path] = new_df
                for why in details['ambiguous']:
                    ambiguous_files.append((path, why))
    ...
    def _schema_resolution_report(self):
        """Compare loaded files to the dominant exact column-name schema.

        The dominant schema is the most frequent exact *set* of labels. Ties
        are resolved by first-loaded file, giving deterministic behavior.
        Column order is intentionally ignored because axis availability uses
        label membership, not CSV/FCS column order.
        """
        paths = list(self.loaded_files.keys())
        if not paths:
            return {'n_files': 0, 'reference_path': None,
                    'reference_count': 0, 'unresolved': []}
        groups = {}
        first_index = {}
    ...
    def _reveal_paths_in_file_manager(self, paths, parent=None, quiet=False):
        """Reveal file locations using Finder, Explorer, or Linux file manager."""
        ...
        try:
            ...
            if sys.platform == 'darwin':
                # `open -R` accepts multiple paths and asks Finder to reveal
                # them rather than opening the files themselves.
                subprocess.Popen(['open', '-R', *existing],
                                 stdout=devnull, stderr=devnull)
            elif os.name == 'nt':
                # Explorer has no reliable cross-version CLI for multi-select.
                # Select exactly when there is one file; for several files,
                # open each unique containing folder so every file is visible.
                if len(existing) == 1:
                ...
                # Linux desktop environments differ. Try the standardized
                # org.freedesktop.FileManager1 interface first (select/reveal),
                # then fall back to opening each containing folder.
    ...
    def _first_axis_resolution_candidate(self):
        """Return a likely canonical name for the resolver, or '' if none.

        Suggestions are based only on label similarity and complementary file
        coverage.  This method never applies a mapping.
        """
        ...
        for i, a in enumerate(names):
            ...
            for b in names[i + 1:]:
                ...
                if score < 0.78:
                    continue
                # A useful alias should improve coverage when the two spellings
                # are considered together; otherwise they probably coexist as
                # separate measurements rather than alternate names.
                union_n = len(pa | pb)
                ...
                if len(pa) > len(pb):
                    ...
                    canonical = a  # stable first-seen tie break
    ...
    def _resolve_axis_alias_target(self, name):
        """Resolve an alias chain defensively; cycles leave the last stable name."""
        cur = name
        seen = set()
        while cur in self.axis_aliases and cur not in seen:
            seen.add(cur)
            nxt = self.axis_aliases.get(cur, cur)
            if nxt == cur:
                break
            cur = nxt
        return cur
    ...
    def _apply_axis_aliases_to_df(self, df, path=''):
        """Apply confirmed label aliases to one DataFrame without touching data.

        Safety rules:
          * If the canonical name already exists in the file, aliases are left
            as additional columns; the canonical channel is already usable.
          * If multiple aliases that map to the same canonical name coexist
            while the canonical name is absent, none of them is renamed.  This
            avoids duplicate column labels and, more importantly, avoids
            silently deciding which scientifically distinct measurement wins.

        Returns (possibly-renamed_df, details_dict).
        """
        if df is None or not self.axis_aliases:
        ...
        by_target = {}
        for col in present:
            target = self._resolve_axis_alias_target(col)
            if target != col:
        ...
        for target, sources in by_target.items():
            if target in present_set:
                # The canonical measurement is already available. Retain any
                # additional similarly named columns rather than deleting or
                # overwriting them, and report the protected collision so the
                # resolver can keep that file visibly unresolved.
                ambiguous.append(
    ...
    def _apply_axis_mapping(self, canonical, variants):
        """Persist and apply a user-confirmed variant -> canonical mapping.

        Only DataFrame column labels are renamed.  Numeric arrays and row order
        are untouched.  The mapping is also retained for Batch Stats so files
        re-read from disk use the same nomenclature resolution.
        """
        ...
        # If an older alias pointed at one of the newly selected variant names,
        # route it to the new canonical target too.  Then make canonical a true
        # root (never an alias to something else).
        for alias, target in list(self.axis_aliases.items()):
            if alias == canonical:
                continue
            resolved = self._resolve_axis_alias_target(target)
            if target in selected or resolved in selected:
                self.axis_aliases[alias] = canonical
        for variant in variants:
            self.axis_aliases[variant] = canonical
        self.axis_aliases.pop(canonical, None)
        ...
        # If the user was already viewing one of the variant labels, keep the
        # same measurement selected under its new canonical label.
        ...
        # Channel labels are part of all hot-cache keys.  Clearing is cheap and
        # ensures no stale array is retained under an obsolete alias.
        self._tc.clear()
        self._gmc.clear()
        self._scatter_cache.clear()
    ...
    def open_axis_name_resolver(self):
        """Open the explicit axis-nomenclature reconciliation dialog."""
    ...
    def _offer_axis_name_resolution(self):
        """Offer the resolver once per distinct heterogeneous loaded schema."""
        if len(self.loaded_files) < 2:
            return
        inventory = self._axis_name_inventory()
        ...
        if not not_shared:
            return
        signature = (n_files, tuple(sorted((str(k), v)
        ...
        if messagebox.askyesno(
                ...
                f"{n_files} loaded files. Because the axis menus use the strict "
                "intersection, a single spelling variant can hide an axis for the whole set."
                + extra +
    ...
    def _load_paths(self, paths: list):
        ...
        alias_notices:  list = []   # user-confirmed mappings reused on new files
        for path in paths:
            if path in self.loaded_files: continue
            try:
                df = self._read_data_file(path)

                # Apply any mappings the user already confirmed earlier in this
                # session before comparing the new file with the loaded schema.
                df, alias_details = self._apply_axis_aliases_to_df(df, path)
                ...
                if self.loaded_files:
                    ref_lower: dict = {}   # casefold → first-seen canonical name
                    for ref_df in self.loaded_files.values():
                        for col in ref_df.columns:
                            ref_lower.setdefault(str(col).casefold(), col)

                    # Group candidates by target so we never manufacture
                    # duplicate column labels when a file already contains the
                    # canonical spelling (or two case variants coexist).
                    by_target = {}
                    for col in df.columns:
                        target = ref_lower.get(str(col).casefold())
                        if target is not None and col != target:
                            by_target.setdefault(target, []).append(col)
                    rename_map = {}
                    present = set(df.columns)
                    for target, sources in by_target.items():
                        if target in present or len(sources) != 1:
                            continue
                        rename_map[sources[0]] = target

                    if rename_map:
                        df = df.rename(columns=rename_map)
                        # Persist the same correction so Batch Stats and future
                        # loads use exactly the nomenclature shown in the UI.
                        self.axis_aliases.update(rename_map)
        ...
        # If exact schemas still differ, offer an explicit reconciliation UI.
        # No fuzzy mapping is applied unless the user confirms it there.
        self._offer_axis_name_resolution()
    ...
    def _file_manager_menu_label(self):
        """Platform-appropriate wording for a single-file reveal action."""
        if sys.platform == 'darwin':
    ...
    def _show_single_file_in_manager(self, path: str):
        """Reveal one exact source path without showing a success popup."""
        if not path or not os.path.exists(path):
            messagebox.showwarning(
                'Show source file',
                f'The source file could not be found at:\n{path}',
                parent=self.root)
            return
    ...
    def _popup_file_context_menu(self, event, path: str):
        """Context menu for an individual file row."""
        menu = tk.Menu(self.root, tearoff=0)
        menu.add_command(
            label=self._file_manager_menu_label(),
    ...
    def _bind_file_context_menu(self, widget, path: str):
        """Bind native right-click plus macOS Control-click to a file widget."""
        callback = lambda e, p=path: self._popup_file_context_menu(e, p)
        widget.bind('<Button-3>', callback, add='+')
        # Tk/macOS users commonly Control-click when no secondary mouse button
        # is configured. Button-2 is retained for older Tk/Aqua mappings.
        if sys.platform == 'darwin':
    ...
    def _add_file_row(self, path: str):
        ...
        # Right-click anywhere on this visual row reveals exactly this source
        # file. Bind each child because Tk events do not bubble to parent Frames.
    ...
    def _exclude_file(self, path: str):
        ...
        self._rebuild_excluded_list()
        # The strict intersection depends on the complete loaded-file set.
        # Recompute it immediately after exclusion so removing the one file
        # with a mismatched name can restore the expected axis.
        self._update_channel_menus()
        self._on_active_files_changed()
    ...
    def _restore_file(self, path: str):
        ...
        self._add_file_row(path)
        self._rebuild_excluded_list()
        # Restoring a file can narrow the shared-axis intersection again.
        self._update_channel_menus()
        self._on_active_files_changed()
    ...
    def _update_channel_menus(self):
        if not self.loaded_files:
            self.x_menu['values'] = ()
            self.y_menu['values'] = ()
            self.x_var.set(''); self.y_var.set('')
            self.x_channel = None; self.y_channel = None
    ...
    def batch_export_stats(self):
        ...
        for fi, fpath in enumerate(target_files):
            ...
            try:
                ...
                # Reuse the exact user-confirmed nomenclature mapping from the
                # interactive analysis.  This closes the previous gap where a
                # case/alias-corrected channel plotted correctly in memory but
                # Batch Stats, after re-reading the raw file, called it missing.
                df, alias_details = self._apply_axis_aliases_to_df(df, fpath)
                if alias_details['ambiguous']:
                    errors.append(
                        f"{os.path.basename(fpath)}: ambiguous axis alias — "
...
class FlowTabManager:
    ...
    def _load_filtered(app, filtered_data, default_x, default_y,
        ...
        # Keep nomenclature resolution identical to the parent.  Sub-gate
        # Batch Stats re-reads raw files, so it needs the same alias map.
    ...
    def open_subgate_tab(self, label: str, filtered_data: dict,
                         ...
                         excluded_files: dict = None, axis_aliases: dict = None):
