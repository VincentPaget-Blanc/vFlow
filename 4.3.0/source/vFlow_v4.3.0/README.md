...
<img src="vflow/docs/ui_layout.svg" alt="vFlow main interface showing sidebar controls, density scatter plot with crosshair gate, region labels, marginal histograms, and statistics panel" width="100%"/>

---

## Changelog

### v4.3.0 — Accuracy, regression and refactor-integrity hardening

- Finalized the v4.3.0 scientific-correctness, interaction, rendering and refactor-integrity audit series through internal checkpoint B6.
- Repaired post-refactor gate-preview wiring, display-scale axis reversal, gate rebinding on transform changes, KDE/Derivative and Otsu API drift, Clear/reload stale state, sub-gate tab cleanup, and degenerate Density/Contour crashes.
- Added permanent generated-data GUI regression, edge-dataset, state-fuzz, refactor-wiring, scientific fingerprint, FlowJo reference and frozen-render validation; the B6 application baseline passes **1,115 automated tests**; the release-ready source adds three metadata checks for **1,118/1,118** passing tests.
- Corrected result-changing edge cases found by an adversarial scientific/accuracy audit while preserving the certified v4.2 reference workflow.
- GMM Multi now emits a separator only when adjacent weighted Gaussian components have a genuine equal-density crossing between their fitted means; degenerate cluster hulls no longer become fabricated bounding-box gates.
- Polar analysis now uses the Zar/CircStat Rayleigh approximation and auto-pairs X/Y centroid columns by channel identity instead of list position.
- Polygon gates fail closed on transform-invalid vertices and use deterministic inclusive edge/vertex membership independent of winding direction. Nonlinear-axis ellipse previews now follow the same raw-data ellipse boundary used for membership/statistics.
- Gate-session loading parses serialized booleans safely and rejects malformed applied geometry/thresholds instead of sanitizing them into different gates.
- CSV/FCS ingestion fails closed on ambiguous duplicate CSV headers, out-of-range integer FCS events, duplicate FCS keywords, malformed supplemental TEXT offsets, and conflicting supplemental metadata; valid supplemental TEXT is now honored before numeric conversion and spillover warning detection.
- Batch Stats/Batch Plot exports now reject overlapping region partitions and output-column naming collisions rather than silently double-counting or overwriting results; family exclusions are limited to explicit SynaptosomesMacro CytoFile families rather than generic underscore-prefix similarity.
- Duplicate applied shape-gate names are disambiguated by gate ID in multi-gate statistics, and invalid shape partitions fail explicitly instead of silently treating all valid events as outside.
- The legacy `biexp`/`logicle` menu choices remain signed-log approximations for backward compatibility; this A1 checkpoint does not redefine those transforms or existing saved gates.

### v4.2.0  Refactor, robustness, nomenclature, and performance

- Completed the behavior-preserving v4.2 structural refactor while retaining the certified v4.1.11 scientific/FCS baseline.
- Hardened transactional gate geometry/state and targeted cache invalidation after the post-refactor behavior review.
- Added session-scoped channel nomenclature resolution, right-click file reveal, and a resizable sidebar.
- Added bounded caches for Density, Contour, marginal histograms, gate masks, and gated-scatter payloads; gate masks are stored compactly and cache retention is byte-budgeted.
- Added bounded parallel cold Density/Contour KDE precomputation across files while keeping Tk/Matplotlib drawing and cache commits on the main thread in deterministic file order.
- Retired the duplicate standalone source launcher. The authoritative release entry point is now the packaged application (`python -m vflow` or `vflow` after installation).
- The legacy `biexp`/`logicle` transform semantics, uncompensated `$SPILLOVER` warning behavior, gate denominators, exports, FCS decoding, gate serialization, and sub-gate lineage remain intentionally unchanged unless explicitly documented otherwise.

### v4.1.11  FlowJo TextToFCS compatibility

- Reads FlowJo TextToFCS v1.3 FCS3.0 files that use trailing TEXT padding and an exclusive one-past-EOF DATA end.
- Missing `$PnE` is inferred as linear `0,0` only when `$PnD` explicitly declares `Linear`; ambiguous files still fail closed.
- Compatibility normalizations are recorded in DataFrame metadata and shown in the application status bar; DATA values are not altered by these metadata repairs.

### v4.1.10  Pre-test reliability hardening

- Installed wheels now include the complete UI implementation; source-tree and installed-package launch paths are validated against the same packaged legacy UI.
- Polar and Batch Plot delayed callbacks are cancelled on window close, preventing destroyed-widget callbacks.
- CSV channels that differ only by letter case fail closed; FCS case-only duplicate labels are disambiguated before DataFrame creation.
- Exact file exclusions and recursive discovery use physical filesystem identity; symlink/hard-link aliases cannot silently double-count the same acquisition.
- Concatenation rejects the same physical CSV selected through multiple aliases, and concat detection prefers `Source_Path` when present.
- Cofactor input rejects non-finite/invalid values and restores the displayed active value; true logarithmic lock-scale limits can no longer cross zero.
- Batch Plot error bars are explicitly labelled and exported as **within-sample binomial counting SE**, not biological replicate SEM.
- Wheel install/startup, source startup, secondary-window lifecycle, and an end-to-end load → gate → stats/gated-data/gate-JSON export → gate reload workflow were smoke-tested under a virtual display.

### v4.1.9  Scientific-correctness baseline

- Gate masks/statistics use one validated finite/displayable event universe; invalid or inconsistent region partitions fail closed.
- Auto-gating rejects empty, degenerate, or unsupported KDE/Otsu splits instead of inventing a numeric fallback threshold.
- CSV/concatenation/gated exports preserve auditable sample provenance with collision-safe `Source_File` plus `Source_Path`.
- Batch Plots correctly combine ordinary files and concatenated files without silently dropping or merging samples.
- The FCS reader now validates offsets/metadata/byte order, escaped TEXT delimiters, mixed and tightly packed integer widths, gain/log scaling, TIME, and truncation. FCS files containing `$SPILLOVER` are loaded with an explicit warning because vFlow does **not** currently apply compensation.
- Legacy `biexp` and `logicle` menu choices are retained numerically for backward compatibility but are vFlow signed-log approximations, **not** standards-compatible Gating-ML Logicle/biexponential transforms. Prefer `asinh` for reproducible quantitative workflows until the standards-compatible transform migration is completed.
...
- **Stacked bar: per-bar binomial SEM.** `_pop_sem_cache` previously stored one SEM per region computed as `std(all_samples)/sqrt(n_samples)`, a global cross-sample aggregate drawn identically on every bar regardless of sample size. Counting SE is now computed per bar as the binomial standard error `sqrt(p*(1-p)/n)`, where `p` is that sample's proportion and `n` is its cell count. `_get_region_pcts_and_n()` was added to return `(pct, n_total)` pairs; `_get_region_pcts()` now wraps it for backward compatibility.
...
- **Sample identity is provenance-aware:** concatenated inputs are split by `Source_Path` when available (falling back to `Source_File` for legacy concatenates), while ordinary loaded files remain separate samples. Mixed ordinary + concatenated selections are supported, and duplicate source names from different containers are disambiguated instead of merged.
...
- **GMM Multi (all crossings, X+Y indep.)**: fits independent 1-D Gaussian Mixture Models on each axis with a user-specified component count (GMM pops X and Y spinboxes, range 1–8, default 3). Every supported equal-density crossing between adjacent components is placed as an individual threshold line; adjacent component pairs that do not actually cross between their fitted means are skipped rather than receiving an artificial separator. All reported crossings appear as independently toggleable checkboxes in the Threshold panel. Requires `scikit-learn`.
...
**Spyder:** open `run_vflow.py` from the project root and press **Run**. The file is only a thin package launcher; all application code remains inside the `vflow` package.
...
### FCS files (`.fcs`, `.FCS`)
Pure-Python list-mode reader, no FCS-specific external dependency. Supports FCS 2.0/3.0/3.1 with strict validation rather than guessing malformed metadata. The reader handles `DATATYPE F`, `D`, and `I`, legal little-/big-endian byte order, mixed integer widths and tightly packed non-byte-aligned integer fields, escaped TEXT delimiters, `$PnE`/`$PnG` scale conversion, and `$TIMESTEP` for TIME parameters. Channel names prefer `$PnS` over `$PnN` and are made unique case-insensitively when labels collide, preventing later cross-file case normalization from collapsing distinct DataFrame columns.

**Compensation:** if `$SPILLOVER` is present, vFlow warns that the matrix exists but does **not** apply it. Quantitative gating of fluorescence channels should therefore use data whose compensation has already been applied/verified upstream. Multiple-data-set FCS files (`$NEXTDATA != 0`) and unsupported/malformed encodings fail closed instead of silently ignoring data.
...
- **Save Only**: concatenates selected CSV files into one pooled file with collision-safe `Source_File` and full `Source_Path` provenance columns, then saves it. Duplicate basenames from different folders remain distinguishable. The dialog remains open.
- **Save & Load**: performs the same provenance-preserving concatenation, saves, then immediately loads the result into the app. Inputs that already contain `Source_File`/`Source_Path` are rejected to prevent nested provenance corruption.
...
| **biexp** | **Legacy vFlow signed-`log1p` approximation; not a standards-compatible biexponential transform** | Backward compatibility only |
| **asinh** | `arcsinh(x / cofactor)` | Recommended transform for mixed positive/negative values in the current release |
| **logicle** | **Legacy vFlow signed-log10 approximation; not Gating-ML Logicle** | Backward compatibility only |

**Cofactor** controls `asinh` and the legacy signed-log `logicle` approximation. Default is 150. Choose and report the value explicitly for reproducibility; do not assume the legacy `logicle` option is interoperable with FlowJo/Gating-ML Logicle coordinates.
...
<img src="vflow/docs/gate_types.svg" alt="Four gate types side by side: crosshair (quadrant grid), rectangle, ellipse, and polygon with vertex handles" width="100%"/>
...
Fits independent 1-D Gaussian Mixture Models on X and Y with a **user-specified component count** (set via the **GMM pops X** and **Y** spinboxes in the sidebar, range 1–8, default 3). Places every supported equal-density crossing between adjacent components as individual threshold lines, all toggleable independently in the **Threshold** panel. If an adjacent pair has no true equal-density crossing between its fitted means, that pair is skipped rather than receiving an artificial threshold.

- **Why user-specified N instead of BIC?** BIC penalises complexity; it almost always merges a small negative cloud into the dominant positive population. Giving direct control over component count makes rare sub-populations discoverable: increase the spinbox by 1 at a time and observe where supported crossings appear, then uncheck any separator that is not biologically useful.
...
<img src="vflow/docs/polar_analysis.svg" alt="Polar analysis window showing overlaid rose histograms for three files with mean-direction arrows and per-file MRL and Rayleigh p-value statistics" width="100%"/>
...
- **Concatenated inputs**: rows are split by `Source_Path` when available, with legacy `Source_File` as fallback, preserving per-acquisition identity.
- **Individual inputs**: every loaded+checked ordinary file remains its own sample.
- **Mixed inputs**: ordinary files and concatenated files can be selected together; neither class is silently discarded, and duplicate source names across containers are disambiguated.
...
100 % stacked bar chart showing gate region percentages per sample. Rendered only when a gate is selected (otherwise only the distribution panel is shown). Optional percentage labels inside bars (for segments ≥ 5 %). The legend is anchored outside the plot area so it does not overlap the bars. Error bars show per-bar binomial **counting standard error**; this is conditional particle-count uncertainty, not biological-replicate SEM.
...
### Export Gated Data → CSV *(v3.9.9; provenance hardened in v4.1.9)*
Saves particle-level data for gated populations. Annotation columns include `Source_File`, `Source_Path`, `Gate_ID`, `Gate_Name`, `Gate_Region`, and `Gate_Type`. Shape gates receive assignment priority before crosshairs so a crosshair cannot absorb every event first. Missing required gate axes fail closed. Particles outside all applied gates are excluded; with no applied gates, active rows are exported with source provenance only.
...
| Coordinate transforms | Cached with dataset generation + file/channels/scales/cofactor, so a changed file at the same path cannot inherit an older transform |
| Gate masks | Cached with dataset generation, channels/scales/cofactor, gate ID and geometry/toggle signature; finite/displayable event validity is part of the computed region universe |
...
vFlow v4.3.0 is distributed as an installable Python package. The packaged entry point is the release authority. Scientific/data logic is separated across `vflow/core`, `vflow/services`, `vflow/plotting`, and `vflow/app`, while the Tk application compatibility layer remains in `vflow/legacy/vflow_app.py` and delegates to those extracted modules.
...
**`BatchPlotWindow`**: dedicated `tk.Toplevel` for per-sample distribution and population % visualisation. Builds provenance-aware samples from ordinary files, concatenated files, or both, preferring `Source_Path` when present and disambiguating collisions. Replots automatically on any parameter change (300 ms debounce).
...
This repository includes both `CITATION.cff` (used by GitHub to display citation guidance) and `.zenodo.json` (used by Zenodo for GitHub-release metadata when both files are present). The public repository is https://github.com/VincentPaget-Blanc/vFlow.

For a specific analysis, cite the **version-specific Zenodo DOI** assigned to the exact vFlow release used. The existing Zenodo vFlow record is part of the same software version chain; create the v4.3.0 deposit as a new version rather than as an unrelated record.

See `CHANGELOG.md` for the cumulative v4.1.4 → v4.3.0 scientific, bug-fix, refactor and validation history.
