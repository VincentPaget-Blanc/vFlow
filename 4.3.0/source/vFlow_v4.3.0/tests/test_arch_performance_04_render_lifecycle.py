...
def test_refresh_plot_delegates_safe_axis_state_reuse_policy():
    source = inspect.getsource(FlowRenderer.render)
    assert "can_preserve_axis_state(" in source
    assert "target_x_scale=plan.x_scale" in source
    assert "target_y_scale=plan.y_scale" in source
