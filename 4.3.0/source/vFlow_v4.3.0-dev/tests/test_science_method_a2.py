...
def test_gate_context_provenance_tracks_logicle_parameters_while_applicability_is_channel_affine():
    state = AnalysisState(x_channel="X", y_channel="Y", x_scale="logicle_gml2", y_scale="linear")
    gate = {"id": 1}
    state.bind_gate_context(gate)
    ...
    current = state.context_dict()
    assert not AnalysisState.contexts_equal(original, current)
    assert state.gate_context_matches(gate)
    assert AnalysisState.contexts_equal(gate["_analysis_context"], current)
