...
def test_gate_evaluation_rebinds_transform_on_same_channels_but_fails_closed_on_channel_change():
    state = linear_state()
    gate = rectangle_gate()
    cache = AnalysisCache()
    regions, _ = evaluate_gate_regions(
    ...
    regions2, colors2 = evaluate_gate_regions(
        gate, [0.0], [0.0], analysis_state=state, analysis_cache=cache)
    assert regions2['IN'].tolist() == [True]
    assert colors2
    assert gate['_analysis_context']['x_scale'] == 'asinh'

    state.x_channel = 'OtherX'
    regions3, colors3 = evaluate_gate_regions(
        gate, [0.0], [0.0], analysis_state=state, analysis_cache=cache)
    assert regions3 == {}
    assert colors3 == []
